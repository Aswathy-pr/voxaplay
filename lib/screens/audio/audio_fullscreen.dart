...
class _AudioFullScreenState extends State<AudioFullScreen> {
  ...
  Future<void> _initializePlaylist() async {
    ...
    if (initialIndex >= 0) {
      ...
      if (mounted) await widget.songService.addToRecentlyPlayed(_currentSong!);
    }
  }
  ...
  void _setupAudioPlayer() {
    ...
    _audioPlayer.currentIndexStream.listen((index) async {
      if (index != null && index < AudioPlayerManager().songs.length && mounted) {
        ...
        await widget.songService.addToRecentlyPlayed(_currentSong!);
      }
    });
  }
  ...
  void _toggleFavorite() {
    setState(() {
      ...
      if (_currentSong != null) {
        ...
        if (songIndex != -1) {
          Hive.box<Song>('songsBox').putAt(songIndex, _currentSong!);
        }
      }
    });
  }
